...
function [sys_size_total, wait_time_total] ...
          = call_yeet_2(arr_rate, serv_time, buffer_cap, ...
...
sys_size_total = zeros(count, batch);
wait_time_total = zeros(count, batch);
...
while (current_time <= sim_time*batch*count+transient)
    ...
    if current_time > transient
        sys_size = [sys_size,[(length(arr_time_in_buffer) - 1)+(length(dep_time_in_group_1) - 1);current_time]];
    end
    
    if current_time > sim_time*count*(iter_2-1) + iter_1*sim_time + transient
        sys_size_total(iter_1,iter_2) = mean(sys_size(1,:));
        wait_time_total(iter_1, iter_2) = mean(wait_time);
        iter_1 = iter_1 + 1;
        sys_size = [];
        wait_time = [];
    end
    
    if current_time > sim_time*count*iter_2 + transient
        iter_2 = iter_2 + 1;
        iter_1 = 1;
    end
